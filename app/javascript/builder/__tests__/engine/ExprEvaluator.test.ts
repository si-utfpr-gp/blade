...
describe("ExprEvaluator", () => {
  describe("arithmetic", () => {
    it("a + b", () => {
      ...
      expect(e.output("a + b", null)).toBe("30")
    })

    it("a - b", () => {
      const e = setup({ a: "50", b: "3" })
      expect(e.output("a - b", null)).toBe("47")
    })

    it("a * b", () => {
      const e = setup({ a: "6", b: "7" })
      expect(e.output("a * b", null)).toBe("42")
    })

    it("a / b", () => {
      const e = setup({ a: "10", b: "3" })
      expect(Number(e.output("a / b", null)).toFixed(2)).toBe("3.33")
    })

    it("floats", () => {
      const e = setup({ a: "3.5", b: "2.1" })
      expect(Number(e.output("a + b", null))).toBeCloseTo(5.6, 1)
    })

    it("integers and floats mixed", () => {
      const e = setup({ a: "10", b: "2.5" })
      expect(e.output("a * b", null)).toBe("25")
    })
  })
  ...
  describe("parentheses", () => {
    it("(a + b) * c", () => {
      ...
      expect(e.output("(a + b) * c", null)).toBe("20")
    })
  ...
  describe("relational", () => {
    it("> true", () => {
      ...
      expect(e.condition("a > b", null)).toBe(true)
    })

    it("> false", () => {
      const e = setup({ a: "3", b: "5" })
      expect(e.condition("a > b", null)).toBe(false)
    })
  ...
  describe("logical Portugol", () => {
    it("'e' (and) true", () => {
      const e = setup({ a: "5", b: "3" })
      expect(e.condition("a > 0 e b > 0", null)).toBe(true)
    })

    it("'e' (and) false", () => {
      const e = setup({ a: "5", b: "0" })
      expect(e.condition("a > 0 e b > 0", null)).toBe(false)
    })

    it("'ou' (or) true", () => {
      const e = setup({ a: "5", b: "0" })
      expect(e.condition("a > 0 ou b > 0", null)).toBe(true)
    })

    it("'ou' (or) false", () => {
      const e = setup({ a: "0", b: "0" })
      expect(e.condition("a > 0 ou b > 0", null)).toBe(false)
    })

    it("'nao' with parentheses", () => {
      const e = setup({ a: "0" })
      expect(e.condition("nao (a > 0)", null)).toBe(true)
    })

    it("'nao' + 'e' mixed", () => {
      const e = setup({ a: "0", b: "5" })
      expect(e.condition("nao (a > 0) e b > 0", null)).toBe(true)
    })
  })
  ...
  describe("boolean literals", () => {
    it("verdadeiro in condition", () => {
      const e = setup({})
      expect(e.condition("verdadeiro", null)).toBe(true)
    })

    it("falso in condition", () => {
      const e = setup({})
      expect(e.condition("falso", null)).toBe(false)
    })

    it("verdadeiro in output", () => {
      const e = setup({})
      expect(e.output("verdadeiro", null)).toBe("true")
    })

    it("falso in output", () => {
      const e = setup({})
      expect(e.output("falso", null)).toBe("false")
    })
  })
  ...
  describe("strings", () => {
    it("concat with double-quoted literal", () => {
      ...
      expect(e.output('"Olá, " + nome', null)).toBe("Olá, João")
    })

    it("concat with single-quoted literal", () => {
      const e = setup({ nome: "Maria" })
      expect(e.output("'Olá, ' + nome", null)).toBe("Olá, Maria")
    })
  })
  ...
  describe("assignment", () => {
    it("simple", () => {
      const e = setup({ x: "0" })
      const changes = e.assign("x = 10", null)
      expect(e.output("x", null)).toBe("10")
      expect(changes).toEqual(["x = 10"])
    })

    it("with expression", () => {
      const e = setup({ x: "0", y: "5" })
      e.assign("x = y + 10", null)
      expect(e.output("x", null)).toBe("15")
    })

    it("multiple statements", () => {
      const e = setup({ a: "0", b: "0" })
      e.assign("a = 1; b = 2", null)
      expect(e.output("a", null)).toBe("1")
      expect(e.output("b", null)).toBe("2")
    })

    it("auto-declares undeclared variable", () => {
      const e = setup({})
      e.assign("x = 42", null)
      expect(e.output("x", null)).toBe("42")
    })
    ...
    it("assigns to array index", () => {
      ...
      ev.assign("notas[0] = 10", null)
      expect(ev.output("notas[0]", null)).toBe("10")
    })
    ...
    it("assigns to array index with index variable", () => {
      ...
      mem.set("i", "0")
      const ev = new ExprEvaluator(mem)
      ev.assign("notas[i] = 7", null)
      expect(ev.output("notas[0]", null)).toBe("7")
    })
  })
  ...
  describe("errors", () => {
    it("undeclared variable", () => {
      const e = setup({})
      expect(() => e.output("x + 1", null)).toThrow("não declarada")
    })
  ...
  describe("mixed expressions", () => {
    it("(a + b) * c - a / b", () => {
      ...
      expect(e.output("(a + b) * c - a / b", null)).toBe("28")
    })
  })
