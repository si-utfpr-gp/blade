...
export interface SimulatorExplainProps {
  steps: ExecutionStep[]
...
export default function SimulatorExplain({ steps, currentStepIndex, isStarted, isFinished }: SimulatorExplainProps) {
  ...
  return (
    <div className="p-3 space-y-3">
      ...
      <div className="rounded-lg border border-primary/20 bg-primary/5 p-3">
        <p className="text-xs leading-relaxed text-foreground">{currentStep.explanation}</p>
      </div>
      {currentStep.changes.length > 0 && (
        <div className="space-y-1">
          <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">O que mudou:</span>
          <div className="space-y-0.5">
            {currentStep.changes.map((change, i) => (
              <div key={i} className="flex items-center gap-1.5 text-xs font-mono bg-secondary/10 rounded px-2 py-1 text-secondary">
      ...
      {!isFinished && (
        <div className="rounded-md bg-muted/50 px-3 py-2">
          <p className="text-[11px] text-muted-foreground italic">{currentStep.nextHint}</p>
        </div>
      )}
    </div>
  )
